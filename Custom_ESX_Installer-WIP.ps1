...
# Date: 2021-01-11
# Last Update: 2022-02-12 (with future ideas)
#
...
# 3. WSL 2
# 4. Ubuntu 20.04 LTS
#    a. genisoimage installation require - 'sudo apt-get install geoisoimage -y'
# 5. ImportExcel: 7.1.0
...
$DataSourcePath = "$ScriptPath\VMware.xlsx" # Path to Excel Workbook as data sources
# Option to absolute datapath
...
for ($esxi=0; $esxi -lt $ESXHostsParameters.Count; $esxi++ ) {
    ...
    #
    $KS_Temp_Stage1 = (Get-Content -Path $KS_Template).Replace("HostPW", $HostPW).Replace("DriveHW", $DriveHW).Replace("HostIP", $HostIP).Replace("HostSubnet", $HostSubnet).Replace("HostGW", $HostGW).Replace("ESXHostname", $ESXHostname).Replace("HostDNS1", $HostDNS1).Replace("HostDNS2", $HostDNS2)
...
$newBootFileContent = (Get-Content $bootFile).Replace($bootFileTitle, "title=Loading Automated kickstart ESXi installer - $time")
...
################################################################################################
#
# Scratch area for future work
# Bootable USB Creation
#
$mediaFormat = Read-Host -Prompt 'Create ESX Installer as an ISO or USB Flashdrive? (I/U)'
if ($MediaFormat.ToLower() -eq 'u') { 
    $mediaFormat = 'usb'
    My-Logger "Query available USB devices ..."
    $USBDriveQuery = Get-Disk | Where-Object -FilterScript {$_.Bustype -eq 'USB'}
    # get-disk | Select-Object -Property *
    # get-volume | Select-Object -Property * #.DriveLetter

    if ($USBDriveQuery -is [array]) {
        Write-host -ForegroundColor Red "More than one USB devices detected. Please select which one to use."
        $a = 1
        foreach ($USBitem in $USBDriveQuery ) {
            $USBsize = [math]::Round($($USBitem.Size/1000/1000/1000))
            Write-host -ForegroundColor Yellow "[$a] $($USBitem.FriendlyName) - $USBsize GB (est)"
            $a++
        }
        $select = Read-host -Prompt "Please pick one from the list above (1 to $($a - 1))"
        while ([array](1..$a) -notcontains $select) { 
        ...
        $USBDrive = $USBDriveQuery.Number[$select - 1]
        $USBDiskName = $USBDriveQuery.FriendlyName[$select - 1]
    } else {
        $USBDrive = $USBDriveQuery.Number
        $USBDiskName = $USBDriveQuery.FriendlyName
    }
    My-Logger "Disk '$USBDiskName' selected ..."
    Write-Host $USBDrive, $USBDiskName, $USBObjectID
} else { 
    $mediaFormat = 'iso'
}

$mediaDrive = Read-Host -Prompt 'Enter the drive letter to the USB Flashdrive you want to use '
$mediaDriveColon =$mediaDrive+':'
$rCommand = "genisoimage -relaxed-filenames -J -R -o $isoDestinationFilePath -b ISOLINUX.BIN -c boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e EFIBOOT.IMG -no-emul-boot $isoSourceFiles"
#$rCommand2 = "mkisofs -relaxed-filenames -J -R -o $isoDestinationFilePath -b isolinux.bin -c boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -b efiboot.img -no-emul-boot $isoSourceFiles"
# -eltorito-platform efi 

# mount -t drvfs '\\server\share' /mnt/share
# mount -t drvfs f: /mnt/f
# https://docs.microsoft.com/en-us/windows/wsl/wsl2-mount-disk
if ($mediaFormat -eq 'usb') {
sudo apt-get install syslinux -y
    $rCommand = 'sudo mount -t drvfs $mediaDriveColon /mnt/$mediaDrive'
    $rCommand = 'sudo mount f: /mnt/f'
    wsl bash -c $rCommand
}
#Get-Disk $USBDrive | Clear-Disk -RemoveData -confirm:$false
#Get-Disk $USBDrive |Initialize-Disk -PartitionStyle MBR -PassThru | New-Partition -AssignDriveLetter -UseMaximumSize
#Get-Disk $USBDrive | New-Partition -AssignDriveLetter -UseMaximumSize  | Format-Volume -FileSystem NTFS -NewFileSystemLabel myUSB
...
begin {}
process {
$result = [string]$line.replace("`r`n","`n")
$result
}
end {}
}
get-content <filename> | dos2unix | out-file -filepath <new filename>